...
fig2, (ax1, ax2, ax3)=pp.subplots(1,3,figsize=(14,6), sharey='row', sharex='row')
axs={'sample441.pickle':ax3, 'sample361.pickle':ax2, 'sample114.pickle':ax1}
ax1.set_ylabel('$\\frac{F}{\lambda N E}$', fontsize=22)
...
for dens in cor_frcs:
    ...
    #        for ret in cor_frc[preload]]
        [axs[dens].plot( cor_frc[preload][ret]['lambda_deexp'], cor_frc[preload][ret]['f_by_NE'], 
...
ax1.set_title('$N_\\mathrm{total}$=114')
ax2.set_title('$N_\\mathrm{total}$=361')
ax3.set_title('$N_\\mathrm{total}$=441')
# legend generation
