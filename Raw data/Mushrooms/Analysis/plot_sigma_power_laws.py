...
for dens in pickles:
    ...
    for preload in [preload for preload in preload_axs if np.any( dfs[preload]['N_attached'] )]:
        ...
        pars[preload]['pow_law']={ret: nu.get_power(cor_frc[preload][ret], (-1.5,-0.5))\
...
fig2, ax2=pp.subplots(1,1,figsize=(9,6))#, gridspec_kw=gridspec_kw)# sharey='row', sharex='row')
axs={'sample441.pickle':ax2, 'sample361.pickle':ax2, 'sample114.pickle':ax2}
ax2.set_ylabel('$\log{(FN^{-1})}$', fontsize=22)
[ax.set_xlabel('$\log(\lambda)$', fontsize=22) for ax in (ax2,)]
...
retmap=lambda ret: 1 if ret==2 else 0.2
...
for dens in cor_frcs:
    ...
    for preload in cor_frc:
        nu.mrks.reset()
        [axs[dens].plot( cor_frc[preload][ret]['log1lambda'], cor_frc[preload][ret]['log1f_by_N'], 
            marker=marks[dens], markevery=4, ms=msmap(ret), lw=0, mew=retmap(ret), mec=clr(preload,minf,maxf), mfc='None', 
            c=clr(preload,minf,maxf), 
            **preload_axs[preload]['plotargs']  ) 
            for ret in cor_frc[preload]]

        [axs[dens].plot( cor_frc[preload][ret]['log1lambda'], 
            cor_frc[preload][ret]['log1lambda']*pars[preload]['pow_law'][ret]['alpha']+pars[preload]['pow_law'][ret]['shift'], 
            c=clr(preload,minf,maxf), lw=retmap(ret) ) for ret in pars_dens[dens][preload]['pow_law']]
legend_lines=[
...
for dens in cor_frcs:
    ...
    #        for ret in cor_frc[preload]]
        ...
        [axs[dens].plot( cor_frc[preload][ret]['lambda'], 
            cor_frc[preload][ret]['F_mushroom'], 
            marker=nu.mrks.next(), markevery=4, ms=msmap(ret), lw=0, mew=retmap(ret), mec=clr(preload,minf,maxf), mfc='None', 
            c=clr(preload,minf,maxf), 
            **preload_axs[preload]['plotargs']  ) 
            for ret in cor_frc[preload]]
...
cb=fig2.colorbar(cm.ScalarMappable(norm=cs.Normalize(maxf,minf), cmap=clrs), 
                ax=ax2, ticks=[10,20,30,40], pad=0.01)
cb.ax.tick_params(direction='out')


fig2.tight_layout()
fig2.savefig("sigma_powerlaw.pdf")
