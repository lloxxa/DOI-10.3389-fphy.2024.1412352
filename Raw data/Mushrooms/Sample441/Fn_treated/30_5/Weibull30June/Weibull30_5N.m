...
NUMBER= [237 230 240 224 239];
N_total= 441;
AT = 0.025*0.025;  %It is the total section m2 
AREA_REAL = AT*NUMBER./N_total;
A1 = AREA_REAL(:,1);
A2 = AREA_REAL(:,2);
A3 = AREA_REAL(:,3);
A4 = AREA_REAL(:,4);
A5 = AREA_REAL(:,5);
                        %%%
Lc = 11.2e-3;           %%% meaning...
S1= -y1./(NUMBER(:,1)); %%% force obtained from the experimental curve in N 
S2= -y2./(NUMBER(:,2));
S3= -y3./(NUMBER(:,3));
S4= -y4./(NUMBER(:,4));
S5= -y5./(NUMBER(:,5));

% in this analysis there seems to be the 'feature' that
% stress = force / area / mushroom
% but the amount of mushrooms is already in the area?
S11= S1(150:300)./A1; 
S22= S2(150:300)./A2;
S33= S3(150:300)./A3;
S44= S4(150:300)./A4;
...
m = 8; %It is a fitting parameter but it is found autoconsistently 
...
hold on
plot(E*D1,Y,'k'); 
plot(E1*D1,S1, 'o', E2*D2,S2, 'x', E3*D3,S3, 's', E4*D4,S4,'>', E5*D5,S5, '^');
...
slop1 = (Pn1(1));
intercept1 = (Pn1(2));
sigma01=exp(-intercept1/slop1); % in N??? really? that'd be weird out of
                                % an exponent

Xn_l2 = log(XN2);
Yn_l2 = log(YN2);
val22  = find(Yn_l2 == real(Yn_l2));
Xn_l2 = Xn_l2(val22);
Yn_l2 = Yn_l2(val22);
...
slop2 = (Pn2(1));
intercept2 = (Pn2(2));
sigma02=exp(-intercept2/slop2); % in N

Xn_l3 = log(XN3);
Yn_l3 = log(YN3);
val23  = find(Yn_l3 == real(Yn_l3));
Xn_l3 = Xn_l3(val23);
Yn_l3 = Yn_l3(val23);
...
slop3 = (Pn3(1));
intercept3 = (Pn3(2));
sigma03=exp(-intercept3/slop3); % in N

Xn_l4 = log(XN4);
Yn_l4 = log(YN4);
val24  = find(Yn_l4 == real(Yn_l4));
Xn_l4 = Xn_l4(val24);
Yn_l4 = Yn_l4(val24);
...
slop4 = (Pn4(1));
intercept4 = (Pn4(2));
sigma04=exp(-intercept4/slop4); % in N

Xn_l5 = log(XN5);
Yn_l5 = log(YN5);
val25  = find(Yn_l5 == real(Yn_l5));
Xn_l5 = Xn_l5(val25);
Yn_l5 = Yn_l5(val25);
...
slop5 = (Pn5(1));
intercept5 = (Pn5(2));
sigma05=exp(-intercept5/slop5); % in N

sigma = [sigma01 sigma02 sigma03 sigma04 sigma05]*1000; % in    
...
annotation(gcf,'textbox',[0.2 0.7 0.3 0.08],...
    'String',{['m = ',num2str(round(slop_m,1)) '\pm',num2str(round(err_slop_m,1))], ['\sigma_o = ',num2str(round(sigma_m,2)), '\pm',num2str(round(err_sigma_m,2)) 'mN']},...
    'FitBoxToText','off');
%liner fit of all vectors
  ...
  annotation(gcf,'textbox',[0.2 0.6 0.28 0.25],...
    'String',{['Lm = ',num2str(round(Lm,4))],['m1 = ',num2str(round(slop1,1)), ', \sigma_1 = ',num2str(round(sigma(:,1),2))], ['m2 = ',num2str(round(slop2,1)), ', \sigma_2 = ',num2str(round(sigma(:,2),2))],.....
    ['m3 = ',num2str(round(slop3,1)),', \sigma_3 = ',num2str(round(sigma(:,3),2))], ['m4 = ',num2str(round(slop4,1)),', \sigma_4 = ',num2str(round(sigma(:,4),2))], ['m5 = ',num2str(round(slop5,1)),', \sigma_5 = ',num2str(round(sigma(:,5),2))]});
hold off
